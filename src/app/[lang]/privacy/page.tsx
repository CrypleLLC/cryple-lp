      ...
      <main style={{ padding: '6rem 2rem', maxWidth: '800px', margin: '0 auto', color: '#667eea', lineHeight: 1.6 }}>
        ...
        {t.sections.map((section: Section, i: number) => (
          <section key={i} style={{ marginBottom: '2rem' }}>
            ...
            <p>{section.content}</p>
            {section.note && <p style={{ marginTop: '1rem' }}>{section.note}</p>}
            ...
            {section.extra && <p style={{ marginTop: '1rem' }}>{section.extra}</p>}
            {section.extra2 && <p style={{ marginTop: '1rem' }}>{section.extra2}</p>}
          </section>
        ))}
      </main>
    </div>
